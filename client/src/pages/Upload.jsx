...
//     try {
//       // Try real API first
//       const response = await uploadAPI.upload(files)
...
//       console.warn('API upload failed, using demo mode:', error)
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        if (response.ok) {
          ...
          // Navigate to flow detail page after last file
          if (i === files.length - 1) {
            setTimeout(() => {
          ...
          toast.error(`Upload failed: ${error.error?.message || 'Unknown error'}`)
        }
      }
    } catch (error) {
      console.error(error)
          ...
          className={`border-dashed border-2 p-6 rounded-lg cursor-pointer mb-6 transition-all duration-300 
            ${
              isDragActive
                ...
                : 'border-zinc-700 bg-zinc-800/40 hover:bg-zinc-800/30'
            }`}
        >
          <input {...getInputProps()} />
          {isDragActive ? (
